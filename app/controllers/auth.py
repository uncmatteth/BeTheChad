    ...
    try:
        ...
        if api_key and api_secret:
            try:
                ...
                current_app.logger.error(f"Twitter OAuth error: {str(e)}")
                flash('Error connecting to Twitter. Using demo login instead.', 'warning')
                # Fall through to demo login
        else:
            current_app.logger.warning("Twitter API credentials not configured. Using demo login.")
            flash('Twitter API not configured. See docs/TWITTER_OAUTH_SETUP.md for instructions.', 'info')
            # Fall through to demo login
        
        # DEMO/DEVELOPMENT LOGIN LOGIC BELOW
...
def twitter_callback():
    ...
    try:
        ...
        if not oauth_verifier or not request_token:
            ...
            current_app.logger.error(f"Twitter callback missing parameters: verifier={oauth_verifier}, token={request_token}")
            return redirect(url_for('auth.login'))
        ...
        try:
            ...
            if not user:
                ...
                current_app.logger.info(f"Created new user from Twitter: {twitter_user.screen_name}")
            ...
            current_app.logger.error(f"Twitter API error in callback: {str(e)}")
            flash('Error processing Twitter authentication', 'danger')
            return redirect(url_for('auth.login'))
            
    except Exception as e:
        current_app.logger.error(f"Error in Twitter callback: {str(e)}")
