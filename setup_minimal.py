...
def install_minimal_dependencies():
    """Install minimal dependencies using requirements_minimal.txt."""
    print_step("Installing minimal dependencies")
    
    # Use our minimal requirements file
    if not os.path.exists("requirements_minimal.txt"):
        print("Error: requirements_minimal.txt not found.")
        return False
    
    return run_command([sys.executable, "-m", "pip", "install", "-r", "requirements_minimal.txt"])
...
def main():
    print("\n" + "="*80)
    print("                   CHAD BATTLES MINIMAL SQLITE SETUP")
    print("="*80)
    print("""
This script will set up a minimal version of Chad Battles with SQLite:
1. Configure the environment to use SQLite
2. Install minimal required dependencies
